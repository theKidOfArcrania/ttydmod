...
static void DemoFontSetColor(GX_COLOR color)
{
#if TTYD_US
	...
	using PFN_GXSetTevColor = void (*)(int stage, GX_COLOR *color);
...
void ConsoleSystem::update()
{
	...
	if (con_mono.value)
	{
		cam_id = CameraId::CAMERA_DEBUG;
	}
	else
	{
		cam_id = CameraId::CAMERA_DEBUG_3D;
	}

	// No alpha cutoff
	dispEntry(
		cam_id, 0, 0.f,
		[](CameraId camId, void *user)
		{
...
void ConsoleSystem::logColor(const char *log_text, GX_COLOR color)
{
	...
	{
		auto chan = (EXIChannel)con_ug_chan.value;
...
void ConsoleSystem::drawLine(int line, const char *text, GX_COLOR color)
{
	// Skip if zero alpha
	if (!color.a)
		return;
	...
	if (mIsMonospace)
	{
		DEMOInitCaption(0, 608, 480);
		DemoFontSetColor(color);
		DEMOPuts(4, 8 + line * 8, 0, text);
	}
	else
	{
		FontDrawStart();
...
void ConsoleSystem::updateUsbGecko()
{
	...
	if (!ugProbe(chan))
	{
		if (con_ug_mode.value == 1)
		{
			...
			con_ug_mode.value = EXIChannel::EXI_CHAN_2;
		}
		return;
	}
	...
	while (true)
	{
		...
		{
			con_ug_mode.value = EXIChannel::EXI_CHAN_1;
		}
		*cmd_end = '\0';
...
void ConsoleSystem::disp()
{
	...
	for (int i = first_visible_log_line; i < num_log_lines; ++i)
	{
		...
		GX_COLOR log_color = log_line.color;
	...
	if (mPromptActive)
	{
		...
		constexpr static GX_COLOR kPromptColor = { 0x32, 0x8B, 0xFF, 0xFF };
